...
curl -v -o /etc/profile.d/welcomeK3s.sh ${templateBaseUrl}artifacts/kubernetes/K3s/welcomeK3s.sh
...
if [[ "$k3sControlPlane" == "true" ]]; then
    ...
    curl -sfL https://get.k3s.io | INSTALL_K3S_EXEC="server" INSTALL_K3S_VERSION=v${K3S_VERSION} K3S_KUBECONFIG_MODE="644" sh -
    if [[ $? -ne 0 ]]; then
