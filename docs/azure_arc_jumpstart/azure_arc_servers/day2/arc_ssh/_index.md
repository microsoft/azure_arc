...
- Once the script has finished its run, you should be able to connect via SSH to the Azure Arc-enabled server by running the below command.
  ...
  > **NOTE: if your Arc-enabled server uses other authentication methods make sure to review the [az ssh arc documentation](https://docs.microsoft.com/cli/azure/ssh?view=azure-cli-latest#az-ssh-arc)**
