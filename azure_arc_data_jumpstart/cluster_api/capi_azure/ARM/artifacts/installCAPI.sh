...
echo $profileRootBaseUrl:${11} | awk '{print substr($1,2); }' >>vars.sh
...
sed -i '12s/^/export profileRootBaseUrl=/' vars.sh
...
# Download global dependencies, local dependencies would be the same but changing the root folder to "${profileRootBaseUrl}"
...
export CLUSTER_TOTAL_MACHINE_COUNT
