...
#     $podStatus = $(if(kubectl get postgresqls -n arc | Select-String "Ready" -Quiet){"Ready!"}Else{"Nope"})
...
Do {
   Write-Host "Waiting for PostgreSQL Hyperscale. Hold tight, this might take a few minutes..."
   Start-Sleep -Seconds 45
   $podStatus = $(if(kubectl get pods -n arc | Select-String $podName | Select-String "Running" -Quiet){"Ready!"}Else{"Nope"})
