...
# az login --service-principal --username $env:SPN_CLIENT_ID --password $env:SPN_CLIENT_SECRET --tenant $env:SPN_TENANT_ID --output none // Will be comment-out upon "Directly Connected" functionality restore 
# az resource delete -g $env:resourceGroup -n $env:ARC_DC_NAME --namespace "Microsoft.AzureArcData" --resource-type "dataControllers" // Will be comment-out upon "Directly Connected" functionality restore
