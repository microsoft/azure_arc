...
$githubApiUrl = "$gitHubAPIBaseUri/repos/$githubAccount/azure_arc/contents/azure_arc_k8s_jumpstart/1417_feature/artifacts/workflows?ref=$githubBranch"
...
Invoke-Command -VMName $VMnames -Credential $Credentials -ScriptBlock {
    ...
    $githubApiUrl = "https://api.github.com/repos/$using:githubAccount/$repoName/contents/azure_arc_k8s_jumpstart/1417_feature/artifacts/L1Files?ref=$using:githubBranch"
    $response = Invoke-RestMethod -Uri $githubApiUrl
    $fileUrls = $response | Where-Object { $_.type -eq "file" } | Select-Object -ExpandProperty download_url
    $fileUrls | ForEach-Object {
        $fileName = $_.Substring($_.LastIndexOf("/") + 1)
