...
echo "==============================================================================================================================================================="
echo "Checking to see if connected8s AZ extension is installed."
if [ -z "$($az extension list --query '[].path' -o tsv | grep connected8s)" ]; then
    echo "The Azure CLI extension for connected8s has not been installed."
    echo -n "I will attempt to register the extension now (this may take a few minutes)..."
    $az extension add --name connected8s >/dev/null
    echo "done."
    echo -n "Verifying the Azure CLI extension exists..."
    if [ -z "$($az extension list --query '[].path' -o tsv | grep connected8s)" ]; then
        echo "error! Unable to add the Azure CLI extension for connected8s. Please remediate this."
        exit 1
    fi
    echo "done."
else
...
if [ ! "$($az connected8s show -g "$RESOURCEGROUP" -n "$ARC" --query provisioningState -o tsv 2>/dev/null)" = "Succeeded" ]; then
    ...
    $az connected8s connect -n $ARC -g $RESOURCEGROUP
