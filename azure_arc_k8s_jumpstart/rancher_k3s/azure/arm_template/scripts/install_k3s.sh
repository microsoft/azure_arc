...
sudo -u $adminUsername az extension add --name connected8s
...
sudo -u $adminUsername az connected8s connect --name $vmName --resource-group $resourceGroup --location 'eastus' --tags 'Project=jumpstart_azure_arc_k8s'
